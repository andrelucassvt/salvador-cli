...
| Principais dependências | Runtime: nenhuma (`dependencies: {}` em `pubspec.yaml`). Dev: `lints ^6.0.0`, `test ^1.25.6`. Em `app/`: `flutter`, `salvador_cli` (path `..`), `flutter_bloc ^9.1.1`, `get_it ^9.2.1`, `file_selector ^1.1.0`, `window_manager ^0.5.2`; dev: `flutter_lints ^6.0.0`, `bloc_test ^10.0.0`, `mocktail ^1.0.5`, `checks ^0.3.1` |
...
WorkspaceCubit ─→ OllamaRepository ─→ OllamaClient ─→ /api/tags, /api/ps, /api/show, /api/generate, /api/version
...
| App desktop | `app/lib/presentation/desktop/` | Clean Architecture com GetIt (`AppInjector`): `WorkspaceCubit`/`ChatCubit`/`FileExplorerCubit`/`SettingsCubit` orquestram conexão HTTP, carga de modelos, configurações, atividade/sessões e árvore/preview; `DesktopView` desenha o shell (top bar, painéis com rails, modal, composer) |
...
| Estado persistido do desktop | `app/lib/common/services/desktop_storage_service.dart` | Serialização JSON versionada de `DesktopPreferencesEntity`/`PersistedSessionSummary` com leitura defensiva e gravação atômica |
...
| Arquitetura do app | `app/lib/config/inject/app_injector.dart`, `app/lib/domain/`, `app/lib/data/`, `app/lib/config/error/` | GetIt registra services → datasources → repositories → cubits; entidades e contratos de Repository no domínio; datasources/repos com `Result<T>` e `AppException` |
| Apresentação do app | `app/lib/presentation/desktop/` | `DesktopView` (shell) + `view_model/*Cubit` + `content/`, `widgets/` e `theme/` |
...
| Testes do app | `app/test/` | Cubits com fakes de Repository (`presentation/desktop/`), RepositoryImpl com fakes de DataSource (`data/`), entidades/erros (`domain/`, `config/`), services (`common/services/`) e `salvador_desktop_app_test.dart` (widget tests do shell) |
...
- **Comandos slash são do frontend.** `/clear`, `/exit` e `/quit` são tratados em `bin/salvador_cli.dart`; no desktop, `_ShellScreenState._send` (`desktop_view.dart`) trata `/exit` e `/quit` com `SystemNavigator.pop()`. Nenhum deles chega ao modelo.
- **Descoberta do Ollama só na CLI.** O desktop resolve tudo por HTTP (`/api/tags`, `/api/ps`, `/api/version`) e não invoca o binário; o runner de caminhos absolutos (`/opt/homebrew/bin`, `/usr/local/bin`, `%LOCALAPPDATA%\Programs\Ollama`) saiu do app desktop e permanece apenas no fluxo da CLI, onde `Process.run` precisa do PATH.
