...
O shell tem quatro regiões: title bar customizada de 38 px no macOS, top bar de 62 px (logo, pasta, modelo, iniciar/encerrar, nova sessão, configurações), painel esquerdo escuro de atividade/sessões com rail de 50 px (navegação Chat/Git), e painel direito claro de árvore de arquivos com rail de 50 px. A área central alterna entre chat/estado vazio e `GitWorkspace`; o conteúdo de um arquivo abre como `FilePreviewDialog` amplo sobre a seção atual, inclusive quando o clique veio do worktree Git. Tudo o que pode falhar (troca de pasta, de modelo, salvar configurações, testar servidor) só persiste após sucesso; o modelo é carregado antes de qualquer envio ao agente (chat ou assistente Git).
...
10. **Árvore e preview** — `presentation/desktop/shared/widgets/files_panel.dart` → `desktop_view.dart` → `presentation/desktop/shared/view_model/file_explorer_cubit.dart` + `data/datasources/workspace_datasource.dart`
    A árvore é indexada com `Directory.listSync(followLinks: false)` dentro de `WorkspaceDataSource.listTree`, ignorando `FileMentionService.ignoredDirectories` e symlinks, ordenada pastas-antes-de-arquivos; filtro case-insensitive achata a visão (`FileExplorerCubit._visibleEntries`). Sem projeto vinculado, `FileExplorerCubit.setRoot(null)` nunca chama o repositório e emite `FileExplorerLoaded` vazio direto; o painel de arquivos mostra "Nenhum projeto vinculado" no lugar da árvore e do rodapé de escopo. Ao clicar em arquivo da árvore, `FilesPanel` delega para `_ShellScreenState._openFilePreview`; o mesmo callback é entregue ao `GitWorkspace` e ao `GitWorktreePanel`. Ele chama `FileExplorerCubit.openPreview`, que lê pela ferramenta `read_file` de um `ToolRegistry` com `AgentPermissions.readOnly` (mesma resolução confinada das ferramentas, dentro de `WorkspaceDataSource`) e produz `FilePreviewEntity` com linguagem, linhas e tamanho (calculado a partir do conteúdo lido, não de um cache da árvore). `FilePreviewDialog` abre amplo sobre o shell, com corpo numerado selecionável e highlight leve; fechar ou mencionar limpa o preview do Cubit. Binário, UTF-8 inválido, caminho fora da raiz, arquivo removido/sem permissão viram `FilePreviewErrorDialog`. "Mencionar com @" insere o caminho no composer com a codificação de aspas para espaços. Com `contextFilesEnabled`, o composer troca para `skillSuggestions` quando o texto começa com `/` sem espaço e `insertSkill` substitui o prefixo pelo nome escolhido.
...
- **Erro no preview:** o resultado de erro do `WorkspaceRepository.readFile` (`FileSystemFailureException`, a partir do `ERRO:` da ferramenta de leitura) vira `previewError` e `FilePreviewErrorDialog` exibe a mensagem com botão de fechar.
...
| Apresentação Chat | `app/lib/presentation/desktop/chat/{content,widgets}/` | Composer, atividade, diálogo de preview, cartões, highlighter e `FileChip` |
| Apresentação Git | `app/lib/presentation/desktop/git/{content,widgets}/` | Workspace, revisão de ação, drawer, refs, grafo, inspector, worktree e encaminhamento do clique de arquivo ao preview compartilhado |
...
- **Sem GoRouter.** O app tem uma única tela (`_ShellScreen`); configurações e preview são modais (`showDialog`), não rotas navegáveis. Introduzir GoRouter para um único destino seria infraestrutura sem uso real — decisão registrada em `docs/plan/salvador-desktop-clean-architecture/00-indice.md`.
