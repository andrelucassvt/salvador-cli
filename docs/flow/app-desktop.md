...
8. **Envio ao agente e propostas Git** — `presentation/desktop/view/desktop_view.dart` → `_ShellScreenState._send` + `presentation/desktop/view_model/chat_cubit.dart` → `ChatCubit.send`
...
| Estado | `app/lib/presentation/desktop/view_model/chat_{cubit,state}.dart` | Mensagens, atividades, envio, sessão em andamento e propostas Git pendentes/executáveis |
