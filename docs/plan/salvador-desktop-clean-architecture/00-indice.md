...
| 3 | `03-workspace-cubit.md` | `WorkspaceCubit` funcional e testado (conexão, modelo, pasta raiz, configurações, persistência) registrado no `AppInjector` | não — expõe o contrato de estado que `ChatCubit`/`FileExplorerCubit` consomem via `MultiBlocListener` na parte 5/6 | 2 | concluída |
| 4 | `04-chat-e-explorer-cubits.md` | `ChatCubit` e `FileExplorerCubit` funcionais e testados, registrados no `AppInjector` | não — mesmo motivo: contrato de estado consumido pela View nas partes 5/6 | 3 | concluída |
| 5 | `05-view-shell-topbar-settings.md` | Shell, top bar, menus, painel/rail de atividade e diálogo de configurações migrados para os Cubits; `main.dart` inicializa o `AppInjector` | não — envolve UI com validação funcional do usuário e compartilha `salvador_desktop_app.dart` com a parte 6 | 4 | concluída (executada junto com a parte 6, ver nota nela) |
| 6 | `06-view-arquivos-composer-limpeza.md` | Painel/rail de arquivos, preview, composer e área de chat migrados; `DesktopController` antigo removido; testes de widget adaptados; flow atualizado | não — mesmo arquivo/UI da parte 5, validação funcional do usuário, remoção de código legado | 5 | concluída |
