...
- **O app desktop resolve tudo por HTTP via `OllamaClient`** (`/api/tags`, `/api/ps`, `/api/show`, `/api/generate`, `/api/version`), com uma exceção: quando a conexão falha e o host é local, `OllamaRepositoryImpl` executa `OllamaDiscovery.listModels()` (`ollama list`, que reabre o servidor em background) e retenta uma vez. A CLI usa `OllamaDiscovery` para listar modelos. Mudança na descoberta do Ollama precisa considerar os dois frontends
