...
- `lib/src/` — módulos planos, um por responsabilidade: `agent`, `ollama_client`, `ollama_discovery`, `tools`, `file_mentions`, `terminal_input`, `config`, `models`, `prompt`
- `lib/salvador_cli.dart` — barrel; único ponto de importação do pacote
- `bin/salvador_cli.dart` — CLI interativa
- `app/lib/src/desktop/` — `DesktopController` (`ChangeNotifier`) e `SalvadorDesktopApp`
- `test/` — testes do pacote; `app/test/` — testes do app
- `docs/flow/` — documentação de fluxos
...
- `./sync-brain.sh` — atualiza as skills Brain Flows em `.claude/skills/` e `.agents/skills/`
...
- Comandos slash (`/clear`, `/exit`, `/quit`) são tratados no frontend e nunca chegam ao modelo
...
- **Limites embutidos:** `read_file` trunca em 100.000 caracteres, `run_command` em 20.000, arquivo mencionado com `@` em 512 KiB, loop do agente em 8 rodadas de ferramentas
- **O app desktop não herda o PATH do shell:** por isso `_runOllamaProcess` em `desktop_controller.dart` tenta caminhos absolutos (`/opt/homebrew/bin`, `/usr/local/bin`, `%LOCALAPPDATA%\Programs\Ollama`). Mudança na descoberta do Ollama precisa considerar os dois frontends
